...
public class ProviderBService implements PaymentProvider {
    ...
    @Retryable(
            ...
            maxAttemptsExpression = "${retry.providerB.maxAttempts}",
            backoff = @Backoff(
                    delayExpression = "${retry.providerB.delay}",
                    multiplierExpression = "${retry.providerB.multiplier}",
                    maxDelayExpression = "${retry.providerB.maxDelay}"
            )
    )
    @CircuitBreaker(
            name = "providerB",
    ...
    public PaymentResponse processPayment(Payment payment) {
        ...
        return switch (outcome) {
            ...
            case 2 -> {
                log.info("Payment processed successfully by Provider A");
                yield new PaymentResponse(payment.getUuid(), "Success", true);
            }
